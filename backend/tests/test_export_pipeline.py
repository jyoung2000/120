...
def test_ass_outline_mode():
    ...
    # outline_width=3 scaled by font_scale=1.0 at 1080p → round(3*1.0*2)=6
    check("Outline width=6", sp1.get("Outline") == "6",
          f"got {sp1.get('Outline')}")
    # Shadow = max(1, min(4, round(6*0.75))) = max(1, min(4, 4)) = 4
...
def test_ass_outline_width_scaling():
    ...
    # At 1080p: font_scale=1.0, outline_width=5 → round(5*1.0*2)=10
    result = generate_ass(
        segments=SAMPLE_SEGMENTS, start_time=10.0, end_time=25.0,
        outline_width=5, video_width=1920, video_height=1080,
    )
    ol = parse_ass_styles(result).get("Speaker 1", {}).get("Outline", "?")
    check("Outline width 5 at 1080p → 10", ol == "10", f"got {ol}")

    # At 9:16 (1080x1920): font_scale=1.0, outline_width=5 → round(5*1.0*2)=10
    result2 = generate_ass(
        segments=SAMPLE_SEGMENTS, start_time=10.0, end_time=25.0,
        outline_width=5, video_width=1080, video_height=1920,
    )
    ol2 = parse_ass_styles(result2).get("Speaker 1", {}).get("Outline", "?")
    check("Outline width 5 at 9:16 → 10", ol2 == "10", f"got {ol2}")
...
def test_ass_value_clamping():
    ...
    # outline_width > 10 → clamped to 10, then scaled: round(10*1.0*2)=20
    result3 = generate_ass(
        segments=SAMPLE_SEGMENTS, start_time=10.0, end_time=25.0,
        outline_width=25,
    )
    ...
    check("outline_width=25 clamped to 10, scaled to 20", ol == 20, f"got {ol}")
